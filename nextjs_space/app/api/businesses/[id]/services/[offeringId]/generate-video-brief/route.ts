...
import { generateExplainerVideoBrief } from '@/lib/service-content-generator';

/**
 * POST /api/businesses/[id]/services/[offeringId]/generate-video-brief
 * Generates a short explainer video brief/script. Sets videoStatus to script_ready.
 */
export async function POST(req: NextRequest, { params }: { params: { id: string; offeringId: string } }) {
  ...
  if (offering.status !== 'confirmed') {
    return NextResponse.json({ error: 'Only confirmed services can generate video briefs' }, { status: 400 });
  }

  try {
    const brief = await generateExplainerVideoBrief(params.offeringId);
    if (!brief) return NextResponse.json({ error: 'Generation failed' }, { status: 500 });
    return NextResponse.json({ ok: true, brief });
  } catch (e: any) {
    console.error('[generate-video-brief]', e?.message);
